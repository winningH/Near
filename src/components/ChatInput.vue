<template>
  <div
    class="px-8 pb-6 dark:bg-gradient-to-b dark:from-transparent dark:to-[#1a1a2e]/30 bg-gradient-to-b from-transparent to-white/50"
  >
    <div class="max-w-3xl mx-auto w-full">
      <div
        ref="inputContainer"
        class="dark:bg-[#1e1e3a] bg-white dark:border-[#2a2a50] border-slate-200 border rounded-2xl px-3 pb-3 pt-2 transition-all"
        @click="focusTextarea"
        @paste="handlePaste"
      >
        <div v-if="attachments.length > 0" class="relative mb-2.5">
          <button
            ...
            class="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-6 h-7 rounded-md dark:bg-[#1e1e3a]/95 dark:border-[#2a2a50] dark:border dark:text-[#a0a0c0] bg-white border-slate-200 border text-slate-500 flex items-center justify-center"
            ...
            <div
              ...
              :class="
                !att.uploading && att.type && att.type.startsWith('image/')
                  ? ''
                  : 'flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg dark:bg-[#252545] bg-slate-100 border border-slate-200 dark:border-[#2a2a50]'
              "
            >
              ...
              >
                <svg
                  class="w-5 h-5 animate-spin text-slate-400 dark:text-[#6a6a8e]"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <circle
              ...
              <div v-else-if="att.type && att.type.startsWith('image/')" class="relative">
                <img
                  :src="safeUrl(att.url)"
                  :alt="att.name"
                  class="w-16 h-16 rounded-md object-cover cursor-pointer bg-white dark:bg-[#252545] border border-slate-200 dark:border-[#2a2a50]"
                  title="点击预览"
              ...
              <template v-else>
                ...
                <div class="flex flex-col">
                  <span
                    class="text-xs font-medium dark:text-[#e8e8f0] text-slate-700 truncate max-w-[100px]"
                    >{{ att.name }}</span
                  >
                ...
                <button
                  class="ml-1 w-4 h-4 rounded-full dark:bg-[#1e1e3a] bg-slate-200 dark:text-[#6a6a8e] text-slate-400 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors"
                  title="删除附件"
                  @click="removeAttachment(index)"
                >
                  <svg
          ...
          <button
            v-if="showScrollButtons"
            class="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-6 h-7 rounded-md dark:bg-[#1e1e3a]/95 dark:border-[#2a2a50] dark:border dark:text-[#a0a0c0] bg-white border-slate-200 border text-slate-500 flex items-center justify-center"
        ...
        <textarea
          ...
          placeholder="发送消息..."
          rows="2"
          class="block w-full bg-transparent border-none outline-none resize-none py-2 px-1 max-h-[150px] text-[15px] leading-relaxed dark:text-[#e8e8f0] text-slate-800 dark:placeholder-[#6a6a8e] placeholder-slate-400 caret-indigo-600 dark:caret-[#a78bfa] disabled:opacity-50"
        ...
        <div class="grid grid-cols-[1fr_auto_1fr] items-center mt-1.5 px-1 gap-2">
          ...
          <span class="text-[11px] dark:text-[#6a6a8e] text-slate-400 whitespace-nowrap"
          ...
          <div class="justify-self-end flex items-center gap-2">
            <button
              ...
              class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors dark:text-[#6a6a8e] dark:hover:bg-[#252545] dark:hover:text-[#e8e8f0] text-slate-400 hover:bg-slate-100 hover:text-slate-600 disabled:opacity-50"
              title="添加图片或文件"
