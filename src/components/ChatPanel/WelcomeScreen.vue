<template>
  <div class="flex-1 flex items-center justify-center">
    <div class="text-center max-w-[600px] px-10 w-full">
      <svg
        class="w-[72px] h-[72px] mx-auto mb-6 dark:text-[#6c63ff] text-indigo-500
      ...
      >
        ...
        <path d="M22 40c3 4 12 4 15 0" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" />
      </svg>

      <h1 class="text-[32px] font-bold mb-2 dark:bg-gradient-to-r dark:from-[#6c63ff] dark:via-[#a78bfa] dark:to-[#f472b6]
      ...
      <transition name="fade" mode="out-in">
        <div v-if="!selectedAction" key="categories" class="grid grid-cols-2 gap-3 max-w-[480px] mx-auto">
          <button
            v-for="action in quickActions"
            :key="action.text"
            class="flex items-center gap-2.5 px-5 py-3.5 rounded-xl transition-all text-left
        ...
        <div v-else :key="selectedAction.text" class="max-w-[480px] mx-auto text-left">
          <button
            class="flex items-center gap-1.5 mb-4 text-sm dark:text-[#6a6a8e] text-slate-400
              dark:hover:text-[#a0a0c0] hover:text-slate-600 transition-colors"
            @click="back"
          >
          ...
          <div class="flex flex-col gap-2">
            <button
              ...
              :key="idx"
              class="text-left px-4 py-3 rounded-xl text-sm transition-all
                dark:bg-[#16213e] dark:border-[#2a2a50] dark:border dark:text-[#a0a0c0]
                dark:hover:border-[#6c63ff] dark:hover:bg-[#252545] dark:hover:text-[#e8e8f0]
                bg-white border-slate-200 border text-slate-600
                hover:border-indigo-500 hover:bg-slate-50 hover:text-slate-800"
  ...
  data() {
    return {
      ...
      quickActions: [
        ...
        {
          icon: '🌍', text: '翻译', desc: '中英互译、润色与术语解释',
          prompts: [
        ...
        {
          icon: '✍️', text: '写文章', desc: '博客、邮件、文案与大纲',
          prompts: [
        ...
        {
          icon: '💡', text: '分析问题', desc: '排查、评估、对比与拆解',
          prompts: [
            '分析前端首屏加载慢的常见原因与排查思路',
