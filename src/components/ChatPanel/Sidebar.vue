  ...
  <div>
    ...
    <aside v-else class="w-[280px] min-w-[280px] h-full dark:bg-[#0f0f23] bg-slate-100 dark:border-[#2a2a50] border-slate-200 border-r flex flex-col transition-all">
      <div class="flex items-center justify-between px-4 py-3 dark:border-b-[#2a2a50] border-b-slate-200 border-b">
        <div class="flex items-center gap-2.5">
          <svg class="w-7 h-7 dark:text-[#6c63ff] text-indigo-500" viewBox="0 0 32 32" fill="none">
            ...
            <path d="M11 20c1.5 2 6 2 7.5 0" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
          </svg>
          <span class="text-lg font-bold dark:bg-gradient-to-r dark:from-[#6c63ff] dark:to-[#a78bfa] bg-gradient-to-r from-indigo-500 to-purple-500 bg-clip-text text-transparent">Near</span>
        </div>
        <div class="flex items-center gap-1">
          ...
          <button
            ...
            @click="$emit('toggle')"
          >
            <svg class="w-[18px] h-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6" /></svg>
          </button>
        </div>
      </div>

      <button
        class="mx-3 mt-3 mb-2 px-3.5 py-2.5 rounded-xl flex items-center gap-2.5 transition-all
      ...
      <div class="flex-1 overflow-y-auto px-2 py-1">
        <div class="text-[11px] font-semibold dark:text-[#6a6a8e] text-slate-400 uppercase tracking-wider px-2 py-2">对话历史</div>
        <div v-if="conversations.length === 0" class="text-center py-10 dark:text-[#6a6a8e] text-slate-400 text-sm">暂无对话</div>
        <div v-else class="flex flex-col gap-0.5">
          ...
          >
            <svg class="w-[18px] h-[18px] flex-shrink-0 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div v-if="renameModal" class="fixed inset-0 dark:bg-black/60 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[100]">
      <div class="dark:bg-[#16213e] bg-white dark:border-[#2a2a50] border-slate-200 border rounded-2xl p-6 w-[380px] shadow-2xl">
        <h3 class="text-lg font-semibold dark:text-[#e8e8f0] text-slate-800 mb-4">重命名对话</h3>
        <input
          v-model="newTitle"
          type="text"
          placeholder="输入新名称"
          class="w-full px-3.5 py-2.5 rounded-lg text-sm outline-none transition-colors
